...
auto parseNinjaLog() -> std::unordered_map<std::string, StartStop> {
    ...
    while (fin >> startMs >> stopMs >> restatMtime >> output >> hash) {
        outputToStartStop[output] = {std::chrono::milliseconds(startMs), std::chrono::milliseconds(stopMs)};
    }
...
auto createdSortedEvents(std::unordered_map<std::string, StartStop> const& outputToStartStop) -> std::vector<Event> {
    ...
    for (auto const& outputAndStartStop : outputToStartStop) {
        events.push_back({outputAndStartStop.second.start, outputAndStartStop.first, true});
        events.push_back({outputAndStartStop.second.stop, outputAndStartStop.first, false});
    }
...
// Iterates the sorted events and for each time point builds a map of all currently active tasks. The time difference from the
// previous event's time divided by the number of currently active tasks is then added as WCTR to all currently active tasks.
//
// When a task has finished, it is added to the finishedTasks which is then sorted by WCTR.
auto processEventsToWallClockTimeResponsibilities(std::vector<Event> const& sortedEvents)
    -> std::vector<WallClockTimeResponsibility> {
    ...
    for (auto const& e : sortedEvents) {
        ...
        if (e.isStart) {
            ...
            finishedTasks.push_back({it->second, e.output});
    ...
    std::sort(finishedTasks.begin(),
              finishedTasks.end(),
              [](WallClockTimeResponsibility const& a, WallClockTimeResponsibility const& b) {
                  return a.time > b.time;
              });
...
void printWallClockTimeResponsibilities(std::vector<WallClockTimeResponsibility> const& wallClockTimeResponsibilities,
    ...
    for (size_t i = 0; i < numLinesToPrint; ++i) {
        ...
        std::cout << std::fixed << std::setprecision(3) << std::setw(10) << wctr.time.count() << " " << std::setw(10)
                  << wallClockTime.count() << " " << std::setprecision(1) << std::setw(8) << parallelism << " " << wctr.output
                  << std::endl;
    }
}
